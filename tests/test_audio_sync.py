...
class TestDetectFirstSound(unittest.TestCase):
    ...
    def test_parses_silence_end(self, mock_run, _):
    ...
    @patch("core.audio_sync.os.path.isfile", return_value=True)
    @patch("core.audio_sync.subprocess.run")
    def test_returns_zero_when_no_silence(self, mock_run, _):
        mock_run.return_value = MagicMock(
            stderr="silencedetect no silence detected here", returncode=0,
        )
        result = _detect_first_sound("/fake/path.mp4")
    ...
    @patch("core.audio_sync.os.path.isfile", return_value=True)
    @patch("core.audio_sync.subprocess.run")
    def test_custom_threshold(self, mock_run, _):
        mock_run.return_value = MagicMock(
            stderr="silence_end: 1.0", returncode=0,
        )
    ...
    @patch("core.audio_sync.os.path.isfile", return_value=True)
    @patch("core.audio_sync.subprocess.run")
    def test_multiple_silence_ends_takes_first(self, mock_run, _):
        mock_run.return_value = MagicMock(
    ...
    def test_file_not_found_returns_zero(self):
        result = _detect_first_sound("/nonexistent/path.mp4")
        self.assertEqual(result, 0.0)
